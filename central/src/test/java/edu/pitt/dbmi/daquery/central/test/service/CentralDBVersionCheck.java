...
public class CentralDBVersionCheck {
	...
	public void test()
	{
		try
		{
			...
			if(ddlVersion == null)
				fail("No db version found for central db in " + ddlFile);
			if(! (upMin < ddlVersion.floatValue() && upMax > ddlVersion.floatValue()))
				fail("Central db ddl version and maximum update version do not match");
		}
